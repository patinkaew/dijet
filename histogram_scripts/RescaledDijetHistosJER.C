...
TH1D *getJER(TProfile2D* p2, TProfile2D *p2x,
	     double eta1, double eta2, TH1D **h1 = 0, TH1D **h1x = 0, bool isMC=false);
...
void DijetHistosJER(string rootdir="/media/storage/nestorma/dijet/rootfiles", string hadddir="../haddfiles") {
  ...
  // Nestor
  //DijetHistosJERs(rootdir + "/jmenano_data_cmb_2022E_"+version+"_19Dec2023.root","Dijet2");
  DijetHistosJERs(rootdir + "/jmenano_data_cmb_2022E_"+version+"_22Sep2023.root","Dijet2");
  //
  //scaleJER = true; // something weird with MC RMS
  DijetHistosJERs(rootdir + "/jmenano_mc_cmb_Summer22MG_"+version+"_22Sep2023.root","Dijet2");
  ...
  string mc22 = rootdir + "/jmenano_mc_cmb_Summer22MG_"+version+"_22Sep2023.root";
...
//		     "2023D_vs_Summer22_"+version+"");
  // Nestor
  //drawDijetHistosJER(rootdir + "/jmenano_data_cmb_2022E_"+version+".root",
    //                 "2022E_"+version+"_19Dec2023");
  drawDijetHistosJER(rootdir + "/jmenano_data_cmb_2022E_"+version+"_22Sep2023.root",mc22,
                     "2022E_"+version+"_22Sep2023");
//
...
void DijetHistosJERs(string file, string dir) {
  ...
  h1jer13 = getJER(p2m0,p2m0x,-1.3,1.3,&h1m0s13,&h1m0xs13, isMC);
  ...
  h1bal13 = getJER(p2m2,p2m2x,-1.3,1.3,&h1m2s13,&h1m2xs13,isMC);
  ...
  h1fsr13 = getJER(p2mo,p2mox,-1.3,1.3,&h1mos13,&h1moxs13,isMC);
  ...
  for (int i = 1; i != h2jer->GetNbinsX()+1; ++i) {
    ...
    h1jer2 = getJER(p2m0,p2m0x,eta,eta,&h1m0s,&h1m0xs,isMC);

    // Alternative longer calculation
    TH1D *h1bal2(0), *h1m2s(0), *h1m2xs(0);
    h1bal2 = getJER(p2m2,p2m2x,eta,eta,&h1m2s,&h1m2xs,isMC);
    TH1D *h1fsr2(0), *h1mos(0), *h1moxs(0);
    h1fsr2 = getJER(p2mo,p2mox,eta,eta,&h1mos,&h1moxs,isMC);
  ...
  if (true) { // adding RC noise
    ...
    if (s.Contains("UL2018")) {
      frc = new TFile("/media/storage/nestorma/dijet/JERCProtoLab/Summer20UL18/JER_noise/RC_noise_UL18.root","READ");
    }
    if (s.Contains("Run2")) {
...
void drawDijetHistosJER(string sd, string sm, string era) {
  ...
  c1->SaveAs(Form("/media/storage/nestorma/dijet/rootfiles/pdf/DijetHistosJER/DijetHistosJER_JERMC_%s.pdf",era.c_str()));
  c2->SaveAs(Form("/media/storage/nestorma/dijet/rootfiles/pdf/DijetHistosJER/DijetHistosJER_JERSF_%s.pdf",era.c_str()));
} // drawDijetHistosJER
...
void drawDijetHistosJERtest() {
  ...
  */
  ...
  c1->SaveAs("/media/storage/nestorma/dijet/rootfiles/pdf/DijetHistosJER_JER13.pdf");
  ...
  { // adding RC noise
    ...
    c2->SaveAs("/media/storage/nestorma/dijet/rootfiles/pdf/DijetHistosJER_JER13_JERwithRC.pdf");
  } // RC noise
  ...
  if (false) {
    ...
    c1->SaveAs("/media/storage/nestorma/dijet/rootfiles/pdf/DijetHistosJER_JER13_DijetAndZJet.pdf");
    c1->SaveAs("/media/storage/nestorma/dijet/rootfiles/pdf/DijetHistosJER_JER13_DijetAndZJet.root");
    ...
    //c2z->SaveAs("/media/storage/nestorma/dijet/rootfiles/pdf/DijetHistosJER_Bal13_DijetAndZJet.pdf");
    c2z->SaveAs("/media/storage/nestorma/dijet/rootfiles/pdf/DijetHistosJER_Bal13_vsDijetJER.pdf");
  } // Z+jet
...
TH1D *getJER(TProfile2D* p2, TProfile2D *p2x,
	     double eta1, double eta2, TH1D **h1, TH1D **h1x, bool isMC) {
	     //double eta1, double eta2, TH1D **h1, TH1D **h1x) { //Original line
	     //The bool isMC was added to the function getJER as the seven parameter. 
  ...
  for (int i = 1; i != h1s->GetNbinsX()+1; ++i) {
    ...
    double err = (jer ? sqrt(pow(rmsy*erry/jer,2) + pow(rmsx*errx/jer,2)
			     + pow(0.001,2)) : 0);
    // Nestor
    // Multiply the JER by a factor of sqrt(2) to match data and MC
    // From the plots, data is below MC
    double n = sqrt(2.);
