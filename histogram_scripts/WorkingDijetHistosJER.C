...
//cout << "flag to identify if it is reading the getJER function or not" << endl;
TH1D *getJER(TProfile2D* p2, TProfile2D *p2x,
	     double eta1, double eta2, TH1D **h1, TH1D **h1x) {
	     //double eta1, double eta2, TH1D **h1, TH1D **h1x, string file, string dir) {
  //cout << "This flag means that the function getJER already passed" << endl;
  ...
  for (int i = 1; i != h1s->GetNbinsX()+1; ++i) {
    ...
    // Last change: add the DijetHistosJERs function to have the MC or not option
    //void DijetHistosJERs(string file, string dir) {
    //TDirectory *curdir = gDirectory;
    ...
    //cout << "flag to identify if it is reading the isMC or not" << endl;
    //TString s(file.c_str());
    //bool isMC = (s.Contains("_mc"));
    //cout << "flag to indicate that it read the isMC" << endl;
    //double n = sqrt(2.);

    //if (isMC) {
    //    h1jer->SetBinContent(i, jer);
    //    h1jer->SetBinError(i, err);
    //    cout << "DijetHistosJERs(\"" << file << "\") (MC): JER" << endl;
    //} else {
    //    h1jer->SetBinContent(i, jer * n);
    //    h1jer->SetBinError(i, err * n);
    //    cout << "DijetHistosJERs(\"" << file << "\") (Data): JER multiplied by n" << endl;
    //}
    h1jer->SetBinContent(i, jer);
    h1jer->SetBinError(i, err);
    //}
