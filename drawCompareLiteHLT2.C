...
void drawCompareLiteHLT2(string run = "2022F_ZB_v2") {

  setTDRStyle();
  TDirectory *curdir = gDirectory;

  const char *crun = run.c_str();
  TFile *f = new TFile(Form("rootfiles/compareLiteHLT2_%s.root",crun),"READ");
  assert(f && !f->IsZombie());
...
void drawCompareLiteHLT_IOVs() {
  ...
  for (int iov = 0; iov != niov; ++iov) {
    ...
    TFile *f = new TFile(Form("rootfiles/compareLiteHLT_%s.root",cfile),"READ");
    assert(f && !f->IsZombie());
